...
const DashboardPage = () => {
  ...
  useEffect(() => {
    // Redirect non-agents and non-admins away from dashboard
    if (!loading && user && !user.is_agent && !user.is_superuser) {
      navigate('/');
    }
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        if (user?.is_agent || user?.is_superuser || user?.is_superuser) {
        ...
        let propertyIndex = 0;
        let visitIndex = (user?.is_agent || user?.is_superuser || user?.is_superuser) ? 1 : 0;
        const reviewIndex = (user?.is_agent || user?.is_superuser || user?.is_superuser) ? 2 : 1;

        if (user?.is_agent || user?.is_superuser) {
          setMyProperties(results[propertyIndex].data.data.properties || []);
          setMyVisitRequests(results[visitIndex].data.data.visit_requests || []);
        } else {
          setMyVisitRequests(results[visitIndex].data.data.visit_requests || []);
        }

        setMyReviews(results[reviewIndex].data.data.reviews || []);
      } catch (err) {
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-xs text-gray-400 mt-2">
                            Requested by: {request.user?.email || 'Guest User'}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm text-gray-500">
